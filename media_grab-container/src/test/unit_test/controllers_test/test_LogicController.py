...
class TestLogicController(unittest.TestCase):
    ...
    def test_getMediaInfoRecordsWithTorrents(
        self, getTorrentRecordsMock, loggingInfoMock
    ):

        fakeTorrentRecords = [
            TorrentRecord(
                "fakeTorrentTitle1", "id1", "fakeInfoHash1", 5, "4", "3"
            ),
            TorrentRecord(
                "fakeTorrentTitle2", "id2", "fakeInfoHash2", 5, "6", "5"
            ),
            TorrentRecord(
                "fakeTorrentTitle3", "id3", "fakeInfoHash3", 5, "8", "7"
            ),
            TorrentRecord(
                "fakeTorrentTitle4", "id4", "fakeInfoHash4", 5, "10", "9"
            ),
        ]
        ...
        # configure mocks
        getTorrentRecordsMock.side_effect = [
            fakeTorrentRecords,
            [],
            fakeTorrentRecords[2:],
        ]
        ...
        actualOutputMediaInfoRecords = (
            LogicController.getMediaInfoRecordsWithTorrents(
                fakeMediaInfoRecords
            )
        )
        ...
        # asserts
        self.assertEqual(
            expectedFakeMediaInfoRecords, actualOutputMediaInfoRecords
        )
        ...
        fakeTorrentRecordsNames = [
            torrent.getName() for torrent in fakeTorrentRecords
        ]
    ...
    @mock.patch(
        "src.controllers.LogicController.getMediaInfoRecordsWithTorrents"
    )
    @mock.patch(
        "src.controllers.CompletedDownloadsController.auditDumpCompleteDir"
    )
    @mock.patch(
        "src.controllers.QueryGenerationController.addTVEpisodeQueriesToMediaInfoRecords"
    )
    @mock.patch("src.interfaces.MediaIndexFileInterface.loadMediaFile")
    def test_runProgramLogic(
        self,
        loadMediaFileMock,
        addTVEpisodeQueriesToMediaInfoRecordsMock,
        auditDumpCompleteDirMock,
        getMediaInfoRecordsWithTorrentsMock,
        qbittorrentInterfaceGetInstanceMock,
        onSuccessfulTorrentAddMock,
    ):
        ...
        fakeMediaInfoRecord1Copy.setTorrentRecord(
            TorrentRecord(
                f"{fakeMediaInfoRecord1.getShowName()}-torrentName",
                "fakeTorrentId",
                "fakeInfoHash",
                5,
                10,
        ...
        fakeMediaInfoRecord2Copy.setTorrentRecord(
            TorrentRecord(
                f"{fakeMediaInfoRecord2.getShowName()}-torrentName",
                "fakeTorrentId",
                "fakeInfoHash",
                5,
                10,
        ...
        fakeMediaInfoRecordsWithTorrentsFiltered = [
            fakeMediaInfoRecord1Copy,
            fakeMediaInfoRecord2Copy,
        ]
        ...
        qbittorrentInterfaceGetInstanceMock.return_value = (
            qbittorrentInterfaceInstanceMock
        )
        qbittorrentInterfaceInstanceMock.initTorrentDownload.side_effect = [
            True,
            False,
        ]
        ...
        addTVEpisodeQueriesToMediaInfoRecordsMock.assert_has_calls(
            addTVEpisodeQueriesToMediaInfoRecordsMockCalls
        )
